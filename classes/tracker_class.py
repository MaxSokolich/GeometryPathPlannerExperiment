...
class VideoThread(QThread):
    change_pixmap_signal = pyqtSignal(np.ndarray, np.ndarray, list, np.ndarray, list)
            ...
            for (botnum,botcolor) in zip(range(len(self.robot_list)), color):
                try:
                    ...
                    if len(targets) > 0:
                        ...
                        tar = targets[-1]
                        cv2.circle(display_frame,(int(tar[0]), int(tar[1])),15,(0,255,255), -1,)
                except Exception:
                    pass
    ...
    def run(self):
        ...
        while self._run_flag:
            ...
            if ret:       
                ...
                self.change_pixmap_signal.emit(displayframe, cell_mask, self.cell_list, robot_mask,  self.robot_list)
